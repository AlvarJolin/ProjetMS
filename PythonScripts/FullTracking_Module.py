    ...
    def findEverything(self, img, draw=True):
        ...
        if self.resultsHands.multi_hand_landmarks:
            for handLms in self.resultsHands.multi_hand_landmarks:
                if draw:
                    self.mpDraw.draw_landmarks(img, handLms, self.mpHands.HAND_CONNECTIONS, self.mpDraw.DrawingSpec(color=(255, 0, 0), thickness=2, circle_radius=2), self.mpDraw.DrawingSpec(color=(234, 8, 255), thickness=2, circle_radius=2))
                    
        if self.resultsPose.pose_landmarks:
            if draw:
                self.mpDraw.draw_landmarks(img, self.resultsPose.pose_landmarks, self.mpPose.POSE_CONNECTIONS, self.mpDraw.DrawingSpec(color=(255, 0, 0), thickness=2, circle_radius=2), self.mpDraw.DrawingSpec(color=(234, 8, 255), thickness=2, circle_radius=2))
